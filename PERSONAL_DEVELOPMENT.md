...
### 5. 配置模式控制
Smart Ollama Proxy 支持三种配置模式，可以通过环境变量 `CONFIG_MODE` 控制：

| 模式 | 行为 | 适用场景 |
|------|------|----------|
| `auto` (默认) | 加载全局配置，如果存在本地配置则合并（本地覆盖全局） | 常规使用，个人开发 |
| `local` | 只加载本地配置文件，如果不存在则报错 | 完全使用本地配置，忽略全局配置 |
| `global` | 只加载全局配置文件，忽略本地配置 | 生产环境，确保配置一致性 |

**使用方法**：
```bash
# 通过环境变量设置配置模式
export CONFIG_MODE=local  # Linux/macOS
set CONFIG_MODE=local     # Windows
...
**日志验证**：
启动服务后，检查日志输出中的"配置模式:"信息，确认当前生效的配置模式。
